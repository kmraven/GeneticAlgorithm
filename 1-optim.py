...
def main(gray=False):
    if gray:
        ...
        code_to_decimal = lambda x: binary_to_decimal(gray_to_binary(x))
    else:
        decimal_to_code = lambda x, length: decimal_to_binary(x, length)
        code_to_decimal = lambda x: binary_to_decimal(x)

    # Initial population
    population = [decimal_to_code(random.randint(0, 2 ** GENE_LENGTH - 1), GENE_LENGTH) for _ in range(POPULATION_SIZE)]
    # Evaluate initial fitness
    fitness_values = [f1(code_to_decimal(individual)) for individual in population]
